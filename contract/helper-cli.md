...
linked musl helper and the bash CLI were installed to `/usr/local/bin` out of
the tarball `scripts/build-linux.sh` produces, the CLI's sha256 matching
`cli/claude-dashboard-cli` byte for byte. No Homebrew and no helper beside the
script, so `cli/claude-dashboard-cli` could only resolve it through the `PATH`
branch. With an empty store, `decrypt` exited 1 with `No accounts found. Run:
claude-dashboard-cli sync`; a deliberately invalid key on `add-key`'s stdin
exited 1 with `Session key not accepted (expired or invalid).` over real TLS,
writing no store — the positive control that makes the rest meaningful, and the
only evidence that rustls and `ring` built for musl work at runtime, since no
test in either suite speaks TLS. A real key then printed `Added: <email> (Max)`,
and the bash CLI rendered the 5h/7d/Fable gauges with a populated `resets`
column, exit 0 with empty stderr. The rendered reset times were cross-checked
against `usage`'s raw `resets_at`: the two agree, allowing for the sub-second
jitter with which the API reports a window boundary (`21:59:59.98` on one call,
`22:00:00.04` seconds later).
